    ...
    <div className="p-8 w-full h-full flex flex-col bg-[#0a0a0a] text-white overflow-y-auto scrollbar-thin font-sans antialiased">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 max-w-5xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white leading-none">Plantillas de WhatsApp</h1>
            <p className="text-gray-400 text-xs mt-1.5">Gestiona y envía plantillas aprobadas por Meta.</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 bg-[#111111] p-1 border border-[#ffffff10] rounded-md">
            <Button
              onClick={() => setActiveTab("templates")}
              className={`h-7 text-xs px-3 rounded-sm border-0 font-medium ${
                activeTab === "templates" ? "bg-white/10 text-white" : "bg-transparent text-gray-400 hover:text-white"
              }`}
            >
              Plantillas
            </Button>
            <Button
              ...
              className={`h-7 text-xs px-3 rounded-sm border-0 font-medium ${
                activeTab === "logs" ? "bg-white/10 text-white" : "bg-transparent text-gray-400 hover:text-white"
              }`}
            >
              Envíos Recientes
            </Button>
          ...
          <Button
            ...
            className="bg-[#111111] hover:bg-white/5 border border-[#ffffff10] text-white font-semibold rounded-md h-9 text-xs flex items-center gap-2 px-3 transition-all duration-150"
          >
      ...
      {activeTab === "templates" ? (
        <div className="border border-[#ffffff10] rounded-md overflow-hidden bg-[#111111] max-w-5xl">
          <Table>
            <TableHeader className="bg-[#0a0a0a]">
              <TableRow className="hover:bg-[#0a0a0a] border-b border-[#ffffff10]">
                ...
                <TableHead className="text-gray-400 font-semibold h-10">Vista Previa de Contenido</TableHead>
                <TableHead className="text-gray-400 font-semibold h-10">Estado</TableHead>
                <TableHead className="text-gray-400 font-semibold h-10 text-right">Acciones</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoadingTemplates ? (
                ...
                templates.map((template) => (
                  <TableRow key={template.id} className="border-b border-[#ffffff05] hover:bg-[#ffffff06] transition-all duration-150">
                    <TableCell className="font-semibold text-white py-3">{template.name}</TableCell>
                    <TableCell className="text-gray-400 py-3 font-mono text-xs max-w-[350px] truncate">
                      {template.content}
                    </TableCell>
                    ...
                    <TableCell className="py-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Button
                          ...
                          className="h-7 w-7 p-0 text-gray-400 hover:text-white rounded-md hover:bg-white/5"
                          title="Previsualizar"
                        >
        ...
        <div className="border border-[#ffffff10] rounded-md overflow-hidden bg-[#111111] max-w-5xl">
          <Table>
            <TableHeader className="bg-[#0a0a0a]">
              <TableRow className="hover:bg-[#0a0a0a] border-b border-[#ffffff10]">
                <TableHead className="text-gray-400 font-semibold h-10">Contacto</TableHead>
                <TableHead className="text-gray-400 font-semibold h-10">Plantilla</TableHead>
                <TableHead className="text-gray-400 font-semibold h-10">Estado</TableHead>
                <TableHead className="text-gray-400 font-semibold h-10">Fecha</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoadingLogs ? (
                ...
                recentLogs.map((log) => (
                  <TableRow key={log.id} className="border-b border-[#ffffff05] hover:bg-[#ffffff06]">
                    ...
                    <TableCell className="py-3 font-semibold text-gray-300">{log.template.name}</TableCell>
                    <TableCell className="py-3">
      ...
      <Dialog open={selectedPreview !== null} onOpenChange={(open) => !open && setSelectedPreview(null)}>
        <DialogContent className="bg-[#111111] border border-[#ffffff10] text-white rounded-md max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-white tracking-tight">Previsualizar Plantilla</DialogTitle>
          </DialogHeader>
          {selectedPreview && (
            <div className="space-y-4 py-2">
              <div className="space-y-1">
                <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest block">Nombre</span>
                <span className="text-sm font-semibold text-white block">{selectedPreview.name}</span>
              </div>
              <div className="space-y-1">
                <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest block">Cuerpo del mensaje</span>
                <div className="p-3 bg-[#1a1a1a] rounded-md border border-[#ffffff08] font-mono text-xs text-gray-300 whitespace-pre-wrap leading-relaxed">
      ...
      <Dialog open={selectedSend !== null} onOpenChange={(open) => !open && setSelectedSend(null)}>
        <DialogContent className="bg-[#111111] border border-[#ffffff10] text-white rounded-md max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-white tracking-tight">Enviar Mensaje de Plantilla</DialogTitle>
          </DialogHeader>
          {selectedSend && (
            <div className="space-y-4 py-2">
              {/* Contact Autocomplete */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold text-gray-300 uppercase tracking-wider">Destinatario</Label>
                <Input
              ...
              {getTemplateVariables(selectedSend.content).length > 0 && (
                <div className="space-y-3 pt-2 border-t border-[#ffffff08]">
                  <Label className="text-xs font-semibold text-gray-300 uppercase tracking-wider block mb-1">Variables del mensaje</Label>
                    ...
                    {getTemplateVariables(selectedSend.content).map((v) => (
                      <div key={v} className="flex items-center gap-3">
                        ...
                        <Input
                          ...
                          className="bg-[#1a1a1a] border-[#ffffff15] text-white focus-visible:ring-whatsapp text-xs h-8"
                        />
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-1.5 pt-2 border-t border-[#ffffff08]">
                ...
                <div className="p-3 bg-[#0d0d0d] rounded-md border border-[#ffffff05] text-xs text-gray-400 leading-relaxed font-mono">
          ...
          <DialogFooter className="mt-2">
            <Button variant="outline" onClick={() => setSelectedSend(null)} className="bg-transparent border-[#ffffff15] text-gray-300 hover:bg-white/5 hover:text-white rounded-md h-8 text-xs">
              Cancelar
            </Button>
            <Button 
