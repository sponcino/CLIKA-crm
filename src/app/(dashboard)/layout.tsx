...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-darkBg text-white font-sans antialiased selection:bg-whatsapp/30">
      {/* Sidebar */}
      <aside className="w-[220px] bg-darkBg border-r border-[#ffffff10] hidden md:flex flex-col flex-shrink-0">
        <div className="h-12 flex items-center px-4 font-bold text-lg tracking-tight border-b border-[#ffffff10]">
          CLIKA<span className="h-1.5 w-1.5 rounded-full bg-whatsapp ml-1 self-center"></span>
        </div>
        
        {/* Navigation via Client Component for instant active states */}
        <SidebarNav />

        {/* Bottom Panel */}
        <div className="p-3 border-t border-[#ffffff10] bg-[#0c0c0c] flex flex-col gap-2">
        ...
        <header className="h-12 flex-shrink-0 bg-darkBg border-b border-[#ffffff10] flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <Avatar className="h-6 w-6 border border-[#ffffff15]">
